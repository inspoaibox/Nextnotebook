...
- 📁 **文件传输** - 支持在设备间快速传输文件
- 💬 **即时消息** - 支持设备间发送文本消息
- 🌐 **双模式连接** - 局域网直连优先，自动降级到中继服务器
- 🔒 **无需登录** - 完全独立于同步系统，即开即用
- 📊 **传输历史** - 保存传输记录，方便查看历史

> 详细使用说明请参考 [局域网传输助手用户指南](docs/lan-transfer/USER_GUIDE.md)

### Excel 表格笔记
- 📊 **多工作表支持** - 每个笔记可包含多个工作表，支持添加、删除、重命名和排序
- 📐 **单元格编辑** - 支持文本、数字、布尔值等多种数据类型
- 🎨 **单元格格式化** - 支持粗体、斜体、字体颜色、背景色、对齐方式
- 🔢 **数字格式** - 支持常规、数字、百分比、货币、日期等格式
- ➕ **行列操作** - 支持插入/删除行列、调整行高列宽、冻结行列
- 📋 **复制粘贴** - 支持单元格和区域的复制、剪切、粘贴
- ↩️ **撤销重做** - 支持最多 50 步操作历史

#### 支持的公式函数
| 函数 | 说明 | 示例 |
|------|------|------|
| `SUM` | 求和 | `=SUM(A1:A10)` |
| `AVERAGE` | 平均值 | `=AVERAGE(B1:B5)` |
| `COUNT` | 计数非空单元格 | `=COUNT(C1:C10)` |
| `MAX` | 最大值 | `=MAX(D1:D10)` |
| `MIN` | 最小值 | `=MIN(E1:E10)` |
| `IF` | 条件判断 | `=IF(A1>100,1,0)` |
| `ROUND` | 四舍五入 | `=ROUND(F1,2)` |

#### 导入导出格式
- **导入**: `.xlsx` (Excel 文件), `.csv` (逗号分隔值)
- **导出**: `.xlsx` (Excel 文件), `.csv` (逗号分隔值)
